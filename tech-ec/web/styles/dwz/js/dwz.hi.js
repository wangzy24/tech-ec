		...
		lookupFilter:function(){
			...
			$box.find(":hidden").each(function(){
				...
				if(_lookup.currentGroup == $input.attr("lookupGroup") 
					...
					result += inputName.substr("pageInfo.".length);
					result += ":";
				...
				switch(field.type){
					...
					case 'lookup':
						...
						html = '<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'
							+ '<input type="text" class="'+field.styleClass+'" name="'+lookupFieldName+'"'+suggestFrag+' size="'+field.size+'"/>'
					...
					case 'enum':
						...
						break;
					case 'date':
						html = '<input type="text" name="'+fieldName+'" class="date '+field.styleClass+'" pattern="'+field.patternDate+'" size="'+field.size+'"/>'
							+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
						break;
					case 'datetime':
						html = '<input type="text" name="'+fieldName+'" class="date '+field.styleClass+'" pattern="'+field.patternDatetime+'" size="'+field.size+'"/>'
							+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
						break;
					default:
						html = '<input type="text" class="'+field.styleClass+'" name="'+fieldName+'" size="'+field.size+'"/>';
						break;
				}
				return '<td>'+html+'</td>';
			}

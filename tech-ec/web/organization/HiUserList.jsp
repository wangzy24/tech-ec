...
<div class="pageHeader">
	<form rel="pagerForm" method="post" action="hiUserList.action?lookup=${lookup}" onsubmit="return dwzSearch(this, '${targetType}');">
	<div class="searchBar">
		<ul class="searchContent">	
			...
			<li>
				<label><hi:text key="性别:" entity="HiUser"/>:</label>
				<hi:search name="pageInfo.f_gender" emu="gender" cssClass="combox"/>
			</li>	
		</ul>
		<div class="subBar">
	...
	<div class="panelBar">
		...
		<c:choose>
			<c:when test="${empty lookup}">
				<authz:authorize ifAnyGranted="HIUSER_SAVE"><li><a class="add" href="<hi:url>hiUserEdit.action?hiUser.id=-1</hi:url>" target="navTab" rel="HiUser"><span><hi:text key="新建" parameterLanguageKeys="人员"/></span></a></li></authz:authorize>
				<authz:authorize ifAnyGranted="HIUSER_DEL"><li><a class="delete" href="<hi:url>hiUserRemoveAll.action?ajax=1</hi:url>" target="removeSelected" title="<hi:text key="确实要删除这些记录吗?"/>"><span><hi:text key="批量删除"/></span></a></li></authz:authorize>
			</c:when>
			<c:otherwise>
		...
		<tbody>
			...
			<tr>
				...
				<td><input name="orderIndexs" value="${item.id}" type="checkbox"></td>
					...
					${item.org.orgName}<authz:authorize ifAnyGranted="HIORG_VIEW"></a></authz:authorize>
					</td>
				...
				<c:choose>
				    ...
				    <authz:authorize ifAnyGranted="HIUSER_DEL">
				      <a class="btnDel" href="<hi:url>hiUserRemove.action?ajax=1&hiUser.id=${item.id}</hi:url>" target="navTabTodo" title="<hi:text key="删除" parameterLanguageKeys="人员"/>"><hi:text key="删除"/></a>
				    </authz:authorize>
				    <authz:authorize ifAnyGranted="HIUSER_VIEW">
				      <a class="btnView" href="<hi:url>hiUserView.action?hiUser.id=${item.id}</hi:url>" target="navTab" rel="HiUser" title="<hi:text key="查看" parameterLanguageKeys="人员"/>"><hi:text key="查看"/></a>
				    </authz:authorize>
				    <authz:authorize ifAnyGranted="HIUSER_SAVE">
				      <a class="btnEdit" href="<hi:url>hiUserEdit.action?hiUser.id=${item.id}</hi:url>" target="navTab" rel="HiUser" title="<hi:text key="编辑" parameterLanguageKeys="人员"/>"><hi:text key="编辑"/></a>
				    </authz:authorize>
					</c:when>
					<c:otherwise>
						<a class="btnSelect" href="javascript:$.bringBack({id:'${item.id}', userName:'${item.userName}', fullName:'${item.fullName}',orgName:'${item.org.orgName}',gender:'${item.gender}'})" title="查找带回"><hi:text key="查找带回" />"><hi:text key="选择"/></a>
					</c:otherwise>
				</c:choose>
				</td>
			</tr>

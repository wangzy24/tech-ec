<#list serviceTool.getEditField(entity) as field>
	...
	<#if field.fieldType == 1 || field.fieldType = 2 || field.fieldType == 3>
			<dt><hi:text key="${field.fieldLabel}" entity="${entity.entityName}"/>：</dt><dd><input type="text" name="${entity.entityName?uncap_first}.${field.fieldName?uncap_first}" class="textInput<#if field.isFull> required</#if><#if serviceTool.getValidator(field)?exists > ${serviceTool.getValidator(field)}</#if>" value="<#noparse>${</#noparse>${entity.entityName?uncap_first}.${field.fieldName?uncap_first}<#noparse>}</#noparse>" <#if field.fieldType = 2> alt="<hi:text key="请输入整数"/>"</#if><#if field.fieldType = 3> alt="<hi:text key="请输浮点数"/>"</#if><#if field.isReadOnly> readonly="readonly" </#if>/></dd>
	...
	<#elseif field.fieldType == 7>
			<dt><hi:text key="${field.fieldLabel}" entity="${entity.entityName}"/>：</dt><dd><hi:select cssClass="combox" emu="${field.enumerationEntity?uncap_first}" name="${entity.entityName?uncap_first}.${field.fieldName?uncap_first}"/></dd>			

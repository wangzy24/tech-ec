			  ...
			  <#list serviceTool.getChild(entity,allServices)  as childEntity>
				<div style="overflow: auto">
				    ...
				    <#list serviceTool.getDisplayFields(childEntity) as childField>
				        ...
				        <#else>
						<#if  childField.fieldType == 8>
								<td>
									<a href="<hi:url>attachmentView.action?attachment.id=<#noparse>${</#noparse>item.itemAtt_attachment.id<#noparse>}</#noparse></hi:url>" target="blank"  >
										<#noparse>${</#noparse>item.<#assign lkpkField = serviceTool.getLookupFKFiled(childEntity, childField)>${lkpkField.fieldName?uncap_first}.fileNameImage<#noparse>}</#noparse>
									</a>
								</td>
						<#else>
